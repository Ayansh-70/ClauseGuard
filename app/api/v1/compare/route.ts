...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      [docA, docB] = await Promise.all([parseDocA(), parseDocB()]);
    } else {
      ...
      // Parse Contract A and Contract B concurrently
      const parseDocA = async (): Promise<StructuredDocument> => {
        if (body.document_a) {
          ...
          return ingestRawText(body.contract_a.raw_text, body.contract_a.file_name || 'Contract_A.txt');
        } else if (typeof body.raw_text_a === 'string' && body.raw_text_a.trim().length > 0) {
          return ingestRawText(body.raw_text_a, body.file_name_a || 'Contract_A.txt');
        } else {
      ...
      const parseDocB = async (): Promise<StructuredDocument> => {
        if (body.document_b) {
          ...
          return ingestRawText(body.contract_b.raw_text, body.contract_b.file_name || 'Contract_B.txt');
        } else if (typeof body.raw_text_b === 'string' && body.raw_text_b.trim().length > 0) {
          return ingestRawText(body.raw_text_b, body.file_name_b || 'Contract_B.txt');
        } else {
